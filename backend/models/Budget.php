...
class Budget {
    ...
    public function getByEventId($event_id) {
        $sql = "SELECT * FROM budgets WHERE event_id = ?";
    ...
    public function getCategories($budget_id) {
        $sql = "SELECT * FROM budget_categories WHERE budget_id = ? ORDER BY categorie";
        $stmt = $this->pdo->prepare($sql);
    ...
    public function create($event_id, $budget_total) {
        // DEBUG
        error_log("Budget::create() appelé avec event_id=$event_id, budget_total=$budget_total");
        
        $sql = "INSERT INTO budgets (event_id, budget_total) VALUES (?, ?) RETURNING id";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$event_id, $budget_total]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        
        error_log("Budget créé avec ID: " . ($result['id'] ?? 'NULL'));
    ...
    public function updateTotal($budget_id, $budget_total) {
        $sql = "UPDATE budgets SET budget_total = ? WHERE id = ?";
        $stmt = $this->pdo->prepare($sql);
    ...
    public function updateCategorie($cat_id, $categorie, $montant_prevu, $montant_reel) {
        $sql = "UPDATE budget_categories 
                SET categorie = ?, montant_prevu = ?, montant_reel = ? 
                WHERE id = ?";
        $stmt = $this->pdo->prepare($sql);
    ...
    public function getTotalPrevu($budget_id) {
        $sql = "SELECT SUM(montant_prevu) as total FROM budget_categories WHERE budget_id = ?";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$budget_id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    public function getTotalReel($budget_id) {
        $sql = "SELECT SUM(montant_reel) as total FROM budget_categories WHERE budget_id = ?";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute([$budget_id]);
        $result = $stmt->fetch(PDO::FETCH_ASSOC);
        return $result['total'] ?? 0;
