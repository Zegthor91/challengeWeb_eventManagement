...
<head>
    <meta charset="UTF-8">
    <title>Prestataires - Gestion</title>
    ...
    <style>
        ...
        .result-info { margin-top: 15px; color: #666; font-size: 14px; }
        ...
        <main class="main-content">
            <div id="app" v-cloak>
                <div class="page-header">
                    ...
                    <a href="ajouter.php" class="btn-primary">+ Ajouter un prestataire</a>
                </div>
                <div class="controls">
                    <div class="search-bar">
                        <div class="search-input-wrapper">
                            <span class="search-icon">🔍</span>
                            <input v-model="searchQuery" type="text" class="search-input" placeholder="Rechercher par nom, type de service, email ou adresse...">
                            <button v-if="searchQuery" @click="searchQuery = ''" class="clear-btn">✕</button>
                        </div>
                    </div>
                    <div class="result-info">{{ filteredPrestataires.length }} prestataire(s) trouvé(s)</div>
                </div>
                <div v-if="loading" class="loading">
                    <div class="loading-spinner"></div>
                    <p>Chargement...</p>
                </div>
                <div v-if="!loading && filteredPrestataires.length > 0" class="table-vue">
                    <table>
                        <thead>
                            <tr>
                                <th @click="sortBy('nom')" :class="{ sorted: sortField === 'nom' }">Nom <span class="sort-icon">{{ getSortIcon('nom') }}</span></th>
                                <th @click="sortBy('type_service')" :class="{ sorted: sortField === 'type_service' }">Type de service <span class="sort-icon">{{ getSortIcon('type_service') }}</span></th>
                                <th>Téléphone</th>
                                <th>Email</th>
                                <th>Adresse</th>
                                <th>Actions</th>
                            ...
                            <tr v-for="p in filteredPrestataires" :key="p.id">
                                ...
                                <td>
                                    <div class="actions-vue">
                                        <a :href="'modifier.php?id=' + p.id" class="btn-action btn-modifier">✏️</a>
                                        <a :href="'supprimer.php?id=' + p.id" @click="confirmDelete($event, p.nom)" class="btn-action btn-supprimer">🗑️</a>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                ...
                <div v-if="!loading && filteredPrestataires.length === 0" class="empty-state">
                    <div class="empty-icon">🏢</div>
                    <h3>Aucun prestataire trouvé</h3>
                    <p v-if="searchQuery">Essayez avec d'autres mots-clés</p>
                    <p v-else>Ajoutez votre premier prestataire!</p>
                </div>
            </div>
        </main>
    </div>
    ...
    createApp({
            ...
            filteredPrestataires() {
                ...
                if (this.searchQuery.trim()) {
                    ...
                    result = result.filter(p =>
                        ...
                        (p.type_service && p.type_service.toLowerCase().includes(query)) ||
        ...
        methods: {
            async fetchPrestataires() {
                this.loading = true;
                try {
                    const response = await fetch('/views/api/prestataires_data.php');
